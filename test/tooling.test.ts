...
test("reports tools bundled with the language server", () => {
  assert.equal(testedReleaseSet.id, "toolchain-signed-2026-08-01-11");
  assert.equal(testedReleaseSet.url, "https://pawnkit.dev/release-sets/toolchain-signed-2026-08-01-11.json");
  assert.equal(tools.find(({ binary }) => binary === "pawnlsp")?.version, "v0.33.84");
  ...
  assert.equal(bundledTools.find(({ label }) => label === "Pawn linter")?.version, "v1.8.20");
});
