...
test("reports tools bundled with the language server", () => {
  assert.equal(testedReleaseSet.id, "toolchain-signed-2026-08-03-5");
  assert.equal(testedReleaseSet.url, "https://pawnkit.dev/release-sets/toolchain-signed-2026-08-03-5.json");
  assert.equal(tools.find(({ binary }) => binary === "pawnlsp")?.version, "v0.34.14");
  assert.equal(tools.find(({ binary }) => binary === "pawn")?.version, "v1.34.19");
  ...
  assert.equal(bundledTools.find(({ label }) => label === "Pawn formatter")?.version, "v1.4.9");
  assert.equal(bundledTools.find(({ label }) => label === "Pawn linter")?.version, "v1.8.36");
});
