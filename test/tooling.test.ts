...
test("reports tools bundled with the language server", () => {
	assert.equal(testedReleaseSet.id, "toolchain-signed-2026-08-03-22");
	assert.equal(testedReleaseSet.url, "https://pawnkit.dev/release-sets/toolchain-signed-2026-08-03-22.json");
	assert.equal(tools.find(({ binary }) => binary === "pawnlsp")?.version, "v0.34.28");
  ...
  assert.equal(bundledTools.find(({ label }) => label === "Pawn linter")?.version, "v1.8.45");
});
