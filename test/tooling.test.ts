...
test("reports tools bundled with the language server", () => {
  ...
  assert.equal(tools.find(({ binary }) => binary === "pawnlsp")?.version, "v0.33.50");
  assert.equal(tools.find(({ binary }) => binary === "pawn")?.version, "v1.4.1");
  ...
  assert.equal(bundledTools.find(({ label }) => label === "Pawn linter")?.version, "v1.7.16");
});
