...
test("reports tools bundled with the language server", () => {
  assert.equal(testedReleaseSet.id, "toolchain-signed-2026-08-01-12");
  assert.equal(testedReleaseSet.url, "https://pawnkit.dev/release-sets/toolchain-signed-2026-08-01-12.json");
  assert.equal(tools.find(({ binary }) => binary === "pawnlsp")?.version, "v0.33.85");
  ...
  assert.equal(bundledTools.find(({ label }) => label === "Pawn linter")?.version, "v1.8.21");
});
