...
test("exposes released CLI and tool management commands", () => {
  ...
  assert.deepEqual(commands, ["pawn.setupProject", "pawn.showProjectHealth", "pawn.check", "pawn.fmt", "pawn.lint", "pawn.doctor", "pawn.build", "pawn.run", "pawn.installTools", "pawn.showToolVersions"]);
  assert.deepEqual(manifest.contributes.taskDefinitions[0].properties.task.enum, ["check", "fmt", "lint", "doctor", "build", "run"]);
});
